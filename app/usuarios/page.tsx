...
import { Users, Search, UserPlus, Calendar, User, RefreshCw } from 'lucide-react'
            ...
            <div className="bg-gray-50 p-6 flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-3 flex-shrink-0">
                        ...
                        <div>
                            <h1 className="text-base font-bold text-slate-800 leading-tight">Usuarios registrados</h1>
                            <p className="text-xs text-emerald-600 font-medium uppercase tracking-wide opacity-70">
                    ...
                    <div className="relative w-56">
                        ...
                        <input
                            ...
                            placeholder="Buscar..."
                            className="w-full pl-8 pr-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs text-gray-600 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600 transition-all"
                        />
                    </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div className="divide-y divide-gray-100">
                        {loading ? (
                            [...Array(3)].map((_, i) => (
                                <div key={i} className="grid grid-cols-12 px-5 py-4 animate-pulse">
                            ...
                            filtered.map((user, index) => {
                                const role = ROLE_STYLE[user.role] ?? { label: user.role, className: 'text-gray-500 bg-gray-50 border-gray-200' }
                                return (
                                    <div key={user.id} className="grid grid-cols-12 px-5 py-3 hover:bg-emerald-50/50 transition-colors duration-150">
                                        <div className="col-span-1 flex items-center text-xs text-gray-300 font-mono">{index + 1}</div>
                                        <div className="col-span-4 flex items-center gap-3 min-w-0">
                                            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-emerald-600 to-emerald-800 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 shadow-sm">
                                                {getInitials(user.name)}
                                            </div>
                                            <span className="text-sm font-medium text-slate-700 truncate">{user.name}</span>
                                        </div>
                                        <div className="col-span-3 flex items-center">
                                            <span className="text-xs text-emerald-700 font-mono bg-emerald-50 px-2 py-0.5 rounded-lg border border-emerald-100 truncate">
                                                @{user.username}
                                            </span>
                                        </div>
